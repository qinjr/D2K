...
def init_logger(config, icc=False):
    ...
    month = cur_time.split('-')[0]
    day = cur_time.split('-')[1]
    LOGROOT = './log/{}/{}'.format(month, day)
    # dir_name = os.path.dirname(LOGROOT)
    ensure_dir(LOGROOT)
    
    if icc:
        logfilename = 'icc-{}-{}-{}.log'.format(config['model'], config['dataset'], cur_time)
    else:
        logfilename = '{}-{}-{}.log'.format(config['model'], config['dataset'], cur_time)
